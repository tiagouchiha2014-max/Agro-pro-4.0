...
export async function pagePropriedades({ pageHeader, pageBody, state, db, toast, loading }) {
    ...
    <div class="row">
      <div class="col">
        <div class="card">
          ...
          <div style="color:var(--muted); font-size:12px; margin-bottom:8px;">Toque em uma safra para marcar como ativa.</div>
  ...
  async function loadAll(){
    ...
    const [safras, fazendas, talhoes] = await Promise.all([
      ...
      state.fazendaId ? db.listTalhoes({ fazendaId: state.fazendaId }) : Promise.resolve({ data:[], error:null })
    ]);
    loading.hide();
    ...
    tbS.innerHTML = (safras.data||[]).map(s => `
      <tr data-safra="${s.id}" style="cursor:pointer">
        <td>${h(s.nome)}</td><td>${s.ativa?"Sim":"Não"}</td>
      </tr>
    `).join("");
    tbS.querySelectorAll("tr[data-safra]").forEach(tr=>{
      tr.addEventListener("click", async ()=>{
        const id = tr.getAttribute("data-safra");
        loading.show("Marcando safra ativa...");
    ...
    tbF.innerHTML = (fazendas.data||[]).map(f => `
      <tr><td>${h(f.nome)}</td><td>${f.ativa?"Ativa":"Inativa"}</td></tr>
    `).join("");

    const tbT = pageBody.querySelector("#tbTalhoes tbody");
    tbT.innerHTML = (talhoes.data||[]).map(t => `
      <tr><td>${h(t.nome)}</td><td>${num(t.area_ha,2)}</td></tr>
    `).join("");
  }
