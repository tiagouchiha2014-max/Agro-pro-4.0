...
export async function pageClima({ pageHeader, pageBody, state, db, toast, loading }) {
  ...
  pageBody.innerHTML = `
    <div class="card">
      ...
      <table class="table" id="tbChuvas">
        <thead><tr><th>Data</th><th>Talhão</th><th>mm</th><th>Fonte</th></tr></thead>
        <tbody></tbody>
      </table>
    </div>
  ...
  async function loadData(){
    ...
    pageBody.querySelector("#tbChuvas tbody").innerHTML =
      (chuvasRes.data||[]).map(r => `
        <tr>
          <td>${r.data || "—"}</td>
    ...
    pageBody.querySelector("#tbAcum tbody").innerHTML =
      talhoes.map(t => `<tr><td>${h(t.nome)}</td><td>${num(acum.get(t.id)||0,2)}</td></tr>`).join("");
  }
  ...
  pageHeader.querySelector("#btnAddChuva").addEventListener("click", async ()=>{
    ...
    const nome = prompt("Talhão (digite exatamente o nome):\n" + talhoes.map(t=>t.nome).join("\n"));
    if(!nome) return;

    const talhao = talhoes.find(t => String(t.nome).toLowerCase() === nome.toLowerCase());
